...
def p_variable(t):
    '''variable : ID saveIDpilaO_np
                | ID CORCHETEIZQ exp CORCHETEDER quadsArray_np multiDiv_np plusMinus_np relationalOp_np and_np or_np 
...
def p_exp(t):
    '''exp : t_exp
            | t_exp multiDiv_np plusMinus_np relationalOp_np and_np or_np OR saveOperadorOr_np exp'''

def p_t_exp(t):
    '''t_exp : g_exp
            | g_exp multiDiv_np plusMinus_np relationalOp_np and_np AND saveOperadorAnd_np t_exp'''

def p_g_exp(t):
    '''g_exp : m_exp
            | m_exp multiDiv_np plusMinus_np relationalOp_np EQUAL saveOperadorRelacional_np m_exp
            | m_exp multiDiv_np plusMinus_np relationalOp_np NOT saveOperadorRelacional_np m_exp
            | m_exp multiDiv_np plusMinus_np relationalOp_np GREATERTHAN saveOperadorRelacional_np m_exp
            | m_exp multiDiv_np plusMinus_np relationalOp_np LESSTHAN saveOperadorRelacional_np m_exp'''

def p_m_exp(t):
    '''m_exp : t
            | t multiDiv_np plusMinus_np MAS saveOperadorMasMenos_np m_exp
            | t multiDiv_np plusMinus_np MENOS saveOperadorMasMenos_np m_exp'''
...
def p_isArray_np(p):
    '''isArray_np : empty'''
    global is_array, tam_array, current_var_type, current_size
...
def p_quadsArray_np(p):
    '''quadsArray_np : empty'''
    global current_var_scope, tam_array
    id_array = p[-4]
    #limit = varsTable.size_array(id_array)
    #print(PilaO)
    cuadruplo.addQuadruple('VER', PilaO[-1], -1, tam_array)
