...
# Función que convierte el valor de la variable con base a su dirección
...
# Función que regresa el resultado de las operaciones aritméticas entre dos valores
...
# Función que regresa el valor de la variable buscando en la memorias por medio de su dirección virtual
def search_dict(address):
    if address in ctes:
        ...
        print('ERROR: no se encontró el valor de la posicion.')
        exit()
    elif contador == 2:
...
# Función que agrega la dirección virtual de una variable junto con su valor en su respectiva memoria
...
# Hacemos uso de linecache para leer línea por línea del archivo y dividimos cada línea cada que lea una coma
while True:
    if linecache.getline("dataVirtualMachine.txt", i) != '#\n':
        ...
        # Se agregan las funciones junto con sus datos al diccionario de funciones
        if cont == 1:
            dict_func[quad[1]] = quad
        
        # Se agregan las constantes junto con sus datos al diccionario de constantes
        if cont == 2:
            value = convert_type(int(quad[0]), quad[1])
            ctes[int(quad[0])] = value

        # Se leen los cuadruplos y se ejecutan dependiendo del tipo de acción que se requiera
        if cont == 3:
            #print(quad)
            #print(ctes, global_mem, local_mem)
            if quad[1] == '=':
                if int(quad[2]) > 19999 and int(quad[2]) < 20000:
                    aux = search_dict(int(quad[2]))
                    value_dict = search_dict(int(aux))
                else:
                ...
                if int(quad[2]) > 19999 and int(quad[2]) < 22000:
                    aux = search_dict(int(quad[2]))
                    value_oper1 = search_dict(int(aux))
                else:
                    value_oper1 = search_dict(int(quad[2]))

                if int(quad[3]) > 19999 and int(quad[3]) < 22000:
                    aux = search_dict(int(quad[3]))
                    value_oper2 = search_dict(int(aux))
                else:
                ...
                if int(quad[3]) > 19999 and int(quad[3]) < 22000:
                    ...
                    value_oper2 = search_dict(int(quad[3]))
                value = convert_type(int(quad[4]), operaciones_arit('equal', value_oper1, value_oper2))
                add_value(int(quad[4]), value)
